...
void GameSetting::checker::init_game() {
    ...
    for(i = 0; i < 8; i++) {
        ...
        for(j = 0; j < 8; j++) {
            if(!((i + j) & 1) && i < 3) {
                board.boxes[i][j] = 0; // player one normal
                ...
                board.boxes[i][j] = 1; // player two normal
...
void GameSetting::checker::start_game(unsigned int game_mode_chosen, gameinfo game_info) {
    /**
...
void GameSetting::checker::load_game(std::string const &filePath) {
    ...
    if(!inputFile.is_open()) {
        ...
        for(i = 7; i >= 0; i--) {
            for(j = 0; j < 8; j++) {
                inputFile >> square;
                board.boxes[i][j] = square;

                if(square < 4) {
                    player = square & 1;
                    board.pieces[player][k[player]].king = square & 2;
                    board.pieces[player][k[player]].cor_x = j;
                    board.pieces[player][k[player]].cor_y = i;
                    k[player]++;
                }
            }
        }
...
void GameSetting::checker::save_game(std::string const &filePath) {
    int i, j;
...
GameSetting::Board GameSetting::checker::transition_board
(move const &move, Board const &board) {
    int i;
    GameSetting::Board tempBoard;
    GameSetting::move tempMove;
    ...
    if(move.x_route[0] > 7 || move.y_route[0] > 7 || move.x_route[1] > 7 || move.y_route[1] > 7
       ...
       || board.boxes[move.y_route[1]][move.x_route[1]] != 4) {
        ...
        // handle promotion
        if(!board.pieces[move.agent_id][srcPiece].king && move.y_route[1] == 7 * ((~move.agent_id) & 1)) {
            tempBoard.pieces[move.agent_id][srcPiece].king = true;
        }

        //update info
        tempBoard.boxes[move.y_route[0]][move.x_route[0]] = 4;
        tempBoard.boxes[move.y_route[1]][move.x_route[1]] = (int(tempBoard.pieces[move.agent_id][srcPiece].king) << 1) | move.agent_id;
        tempBoard.pieces[move.agent_id][srcPiece].cor_x = move.x_route[1];
        tempBoard.pieces[move.agent_id][srcPiece].cor_y = move.y_route[1];

        return tempBoard;
    }

    else if(abs(xDiff = (move.x_route[1] - move.x_route[0])) == 2 && abs(yDiff = (move.y_route[1] - move.y_route[0])) == 2) {
        ...
        do {
            ...
            xJmp = tempMove.x_route[0] + (0 < xDiff) - (xDiff < 0);
            yJmp = tempMove.y_route[0] + (0 < yDiff) - (yDiff < 0);

            for(i = 0; i < 12; i++) {
                if(    tempBoard.pieces[opponent][i].cor_x == xJmp
                       && tempBoard.pieces[opponent][i].cor_y == yJmp) {
                    jmpPiece = i;
                }
            }
            ...
            if(!board.pieces[move.agent_id][srcPiece].king && tempMove.y_route[1] == 7 * opponent) {
                tempBoard.pieces[move.agent_id][srcPiece].king = true;
            }

            tempBoard.boxes[tempMove.y_route[0]][tempMove.x_route[0]] = 4;
            tempBoard.boxes[yJmp][xJmp] = 4;
            tempBoard.boxes[tempMove.y_route[1]][tempMove.x_route[1]] = (int(tempBoard.pieces[move.agent_id][srcPiece].king) << 1) | move.agent_id;

            tempBoard.pieces[move.agent_id][srcPiece].cor_x = 0xFFU;
            tempBoard.pieces[move.agent_id][srcPiece].cor_y = 0xFFU;
            tempBoard.pieces[opponent][jmpPiece].cor_x = 0xFFU;
            tempBoard.pieces[opponent][jmpPiece].cor_y = 0xFFU;
            tempBoard.pieces[move.agent_id][srcPiece].cor_x = tempMove.x_route[1];
            tempBoard.pieces[move.agent_id][srcPiece].cor_y = tempMove.y_route[1];

            if(!board.pieces[move.agent_id][srcPiece].king && tempBoard.pieces[move.agent_id][srcPiece].king) {
                this->move_since_capture = 0;
                return tempBoard;
            }

            for(i = 0; tempMove.x_route[i] >= 0 && tempMove.x_route[i] <= 7 && tempMove.y_route[i] >= 0 && tempMove.y_route[i] <= 7; i++) {
                tempMove.x_route[i] = tempMove.x_route[i + 1];
                tempMove.y_route[i] = tempMove.y_route[i + 1];
            }
        } while(!(tempMove.x_route[0] > 7 || tempMove.y_route[0] > 7 || tempMove.x_route[1] > 7 || tempMove.y_route[1] > 7
                  || ((tempMove.x_route[0] + tempMove.y_route[0]) & 1) || ((tempMove.x_route[1] + tempMove.y_route[1]) & 1)
                  || tempBoard.boxes[tempMove.y_route[1]][tempMove.x_route[1]] != 4)
                && (abs(xDiff = (tempMove.x_route[1] - tempMove.x_route[0])) == 2 && abs(yDiff = (tempMove.y_route[1] - tempMove.y_route[0])) == 2));
...
std::string GameSetting::checker::pretty_print_moves(move const &move) {
...
void GameSetting::checker::print_board () {
    ...
    for(i = 7; i >= 0; i--) {
        ...
        if (i == 1) {
            ...
            double move_time = ((double) this->players[0].get_total_move_time().count()) * GameSetting::clock::period::num / GameSetting::clock::period::den;
        ...
        if(i == 7) {
            ...
            std::cout << " * Total move time: " << (((double) this->players[1].get_total_move_time().count()) * GameSetting::clock::period::num / GameSetting::clock::period::den) << "s";
        }
        std::cout << std::endl;
        ...
        if(i == 1) {
            ...
            double prev_time = ((double) this->players[0].get_prev_move_time().count()) * GameSetting::clock::period::num / GameSetting::clock::period::den;
            printf ("%-*.*s", 0, 25, "  * Previous move time:");
            std::cout << std::setprecision(2) << std::setw(7) << std::fixed << prev_time << "s";
            std::cout << std::setw(1) << "";
        } else if(i == 7) {
            double total_time = ((double) this->total_move_time.count()) * GameSetting::clock::period::num / GameSetting::clock::period::den;
        ...
        if(i == 7) {
            ...
            std::cout << "  * Previous move time : " << (((double) this->players[1].get_prev_move_time().count()) * GameSetting::clock::period::num / GameSetting::clock::period::den) << "s";
        }
        std::cout << std::endl;
        ...
        if (i == 1) {
            ...
            double prev_time = ((double) this->prev_move_time.count()) * GameSetting::clock::period::num / GameSetting::clock::period::den;
